...
public class VoxelHammer{
    ...
    private void init() throws IOException {
        ...
        glfwSetFramebufferSizeCallback(window, fbCallback = new GLFWFramebufferSizeCallback() {
            public void invoke(long window, int width, int height) {
                if (width > 0 && height > 0 && (VoxelHammer.this.width != width || VoxelHammer.this.height != height)) {
                    VoxelHammer.this.width = width;
                    VoxelHammer.this.height = height;
                    VoxelHammer.this.resetFramebuffer = true;
                }
            }
        });
        ...
        glfwSetCursorPosCallback(window, cpCallback = new GLFWCursorPosCallback() {
            public void invoke(long window, double x, double y) {
                if (mouseDown) {
                    float deltaX = (float) x - VoxelHammer.this.mouseX;
                    float deltaY = (float) y - VoxelHammer.this.mouseY;
                    VoxelHammer.this.viewMatrix.rotateLocalY(deltaX * 0.01f);
                    VoxelHammer.this.viewMatrix.rotateLocalX(deltaY * 0.01f);
                }
                VoxelHammer.this.mouseX = (float) x;
                VoxelHammer.this.mouseY = (float) y;
            }
        });
        ...
        glfwSetMouseButtonCallback(window, mbCallback = new GLFWMouseButtonCallback() {
            public void invoke(long window, int button, int action, int mods) {
                if (action == GLFW_PRESS) {
                    VoxelHammer.this.mouseDown = true;
                } else if (action == GLFW_RELEASE) {
                    VoxelHammer.this.mouseDown = false;
                }
            }
        });
        ...
        /* Load OBJ model */
        byte[] bytes = readSingleFileZip("shaders/scene.obj.zip");
    ...
    private static byte[] readSingleFileZip(String zipResource) throws IOException {
        ZipInputStream zipStream = new ZipInputStream(
                VoxelHammer.class.getClassLoader().getResourceAsStream(zipResource));
    ...
    private void createQuadProgram() throws IOException {
        ...
        int vshader = Utils.createShader("shaders/quad.vs.glsl",
                GL_VERTEX_SHADER);
        int fshader = Utils.createShader("shaders/quad.fs.glsl",
                GL_FRAGMENT_SHADER);
    ...
    private void createComputeProgram() throws IOException {
         ...
         */
        int program = glCreateProgram();
        int cshader = Utils.createShader("shaders/raytracing.glsl",
                GL_COMPUTE_SHADER);
        int geometry = Utils.createShader("shaders/geometry.glsl",
                GL_COMPUTE_SHADER);
